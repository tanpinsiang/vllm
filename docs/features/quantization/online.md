...
### Requantize ModelOpt MXFP8 linears

Serialized ModelOpt MXFP8 linears can be converted to
per-channel-weight/per-token-activation FP8 at load time on platforms with a
compatible FP8 PTPC kernel:
...
remains on checkpoint quantization. The regular linear-kernel selector chooses
the compatible backend and reports an error when no PTPC kernel is available.
